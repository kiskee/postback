...
// Import the initial component
import Postback from "./Postback";

const Job = () => {
  // make the constant to invoke in apply button
  const postback = () => {
    return Postback();
  };

  return (
    <div className="container bg-info text-white col-sm-6">
      <h1 className="text-dark">Job posting to test postback</h1>
      ...
      <button className="btn btn-lg btn-danger" type="button" href='#'
      // button call to the constant
      onClick={() => postback()}
      
      >
        <a href="/form1">Apply</a>
      </button>
    </div>
  );
};
